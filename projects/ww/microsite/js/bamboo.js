...
* With modifications by Ethan Herr - 8/2013 - Weight Watchers Inc
*/
...
var Bamboo = (function (window, document) {
  ...
  Bamboo = function (opts) {
    ...
    this.options = {
      ...
      menuWidth: 265,
      headerHeight: 50,
      ltIE9: false, //support browsers that don't support 'translate'
      snapThreshold: null,
      resize: null,
      start: null,
      move: null,
    ...
    openButton.on(startEvent, this._start.bind(this) );
    openButton.on(moveEvent, this._move.bind(this) );
    openButton.on(endEvent, this._end.bind(this) );
  ...
  Bamboo.prototype = {
      ...
      layout: function(){
        // mobile / tablet
        if (this.info.docWidth < this.options.breakpoint || this.options.breakpoint == null) {
          this.desktop = false;
          // container
    ...
    _start: function(e) { 
      ...
      this.tgt = $(e.currentTarget);
      container.css({ 'transition-duration' : '0s' });

      if(this.options.start) this.options.start(this);
    },
    ...
    _move: function(e) {
      ...
      if(this.options.move) this.options.move(this);
    ...
    _end: function(e) {
      ...
      if(this.options.end) this.options.end(this);

      return false;
    },

    _animateTo: function(x,to){
      if(this.options.ltIE9){
  ...
  // if iOS figure out the address bar height offset
  function testOffset(){
    var offset = 0;
